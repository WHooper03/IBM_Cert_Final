...
def scatter_chart(site, slider):
    df = spacex_df
    if site == 'ALL':
        fig = px.scatter(df, x="Payload Mass (kg)", y='class', color="Booster Version Category")
        return fig
    else:
        data3 = df[df['Launch Site']==(site)]
        fig = px.scatter(data3, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig
